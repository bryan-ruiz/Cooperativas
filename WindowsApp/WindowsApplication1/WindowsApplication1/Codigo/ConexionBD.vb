...
Public Class ConexionBD
    ...
    Function actualizarIngreso(fechap As DateTime,
        ...
        Try
            ...
            SQL = "UPDATE [INGRESOS] SET fecha = '" & fechap & "', " & "reciboFactura = '" & facturaRecibop & "', " & "descripcion = '" & descripcripcionp & "', " & "cantidad = '" & cantidadp & "',  
            " & "precioUnitario = '" & precioUnitariop & "', " & "total = '" & totalp & "', " & "codigoDeCuenta = '" & codCuentap & "' WHERE ((cliente) = '" & clientep & "' )"
    ...
    Function actualizarGasto(fechap As DateTime,
        ...
        Try
            ...
            SQL = "UPDATE [GASTOS] SET fecha = '" & fechap & "', " & "reciboFactura = '" & facturaRecibop & "', " & "descripcion = '" & descripcripcionp & "', " & "cantidad = '" & cantidadp & "',  
            " & "precioUnitario = '" & precioUnitariop & "', " & "total = '" & totalp & "', " & "codigoDeCuenta = '" & codCuentap & "' WHERE ((proveedor) = '" & clientep & "' )"
    ...
    Function eliminarFacturaIngresos(ByVal proveedor As String) As Integer
        Dim res As Integer = 0
        Try
            SQL = "DELETE FROM [INGRESOS]" &
            "WHERE cliente = ('" + proveedor + "')"
    ...
    Function eliminarFacturaGastos(ByVal proveedorp As String) As Integer
        Dim res As Integer = 0
        Try
            SQL = "DELETE FROM [GASTOS]" &
            "WHERE proveedor = ('" + proveedorp + "')"
    ...
    '@Return - List of String
    '@numAsociado - num del socio
    Function obtenerNumAsociadoDeSocios(ByVal idNumAsociado As String) As List(Of String)
        Dim MyList As New List(Of String)
        Try
            SQL = "SELECT nombre FROM [SOCIOS]" &
                "WHERE numAsociado = ('" + idNumAsociado + "')"
    ...
    '@Return - List of String
    '@id - cedula del socio
    Function obtenerNumCedulaDeAsociado(ByVal id As String) As List(Of String)
        Dim MyList As New List(Of String)
        Try
            SQL = "SELECT nombre FROM [SOCIOS]" &
                "WHERE cedula = ('" + id + "')"
    ...
    'Selecciona nombre de un usuario con estado Retirado 
    Function consultarAsociadoRetiradoXCedOrNumAsoc(ByVal cedulaONumAsociado As String) As List(Of String)
        Dim MyList As New List(Of String)
        Try
            SQL = "SELECT SOCIOS.nombre
                    FROM [SOCIOS]
                    WHERE ((SOCIOS.cedula) = '" & cedulaONumAsociado & "' or (SOCIOS.numAsociado)= '" & cedulaONumAsociado & "')
                    AND ((SOCIOS.estado) = 'Retirado') "
    ...
    'Selecciona nombre de un usuario con estado Activo 
    Function consultarAsociadoActivoXCedOrNumAsoc(ByVal cedulaONumAsociado As String) As List(Of String)
        Dim MyList As New List(Of String)
        Try
            SQL = "SELECT SOCIOS.nombre
                    FROM [SOCIOS]
...
" & "fechaVence = '" & fechaVence & "', " & "cedula = '" & cedula & "' WHERE ((tipoConsejo) = '" & tipoConsejo & "' and (tipo) = '" & puesto & "' )"
    ...
    Function obtenerDatosdeConfigurationConsecutivoAsociado() As List(Of ConsecutivoAsociadoClase)
        Dim MyList As New List(Of ConsecutivoAsociadoClase)
        Try
            SQL = "SELECT CONSECUTIVO_ASOCIADO.* FROM [CONSECUTIVO_ASOCIADO]"
            'pregunto antes si estoy conectado a la base de datos'
            If conectadoBD = True Then
                Dim command As New OleDbCommand(SQL, objConexion)
                Dim reader = command.ExecuteReader()
...
" & "Institucional = '" & institucional & "', " & "Patrimonial = '" & patrimonial & "' "
    ...
    Function actualizarConsecutivoAsociado(ByVal consecutivo As Integer) As Integer
        ...
        Try
            'Declaramos el query que queremos ejecutar
            SQL = "UPDATE [CONSECUTIVO_ASOCIADO] SET consecutivo = '" & consecutivo & "' "
...
" & "fechaLimite10 = '" & fecha10 & "' "
    ...
    Function insertarLicencia(ByVal codigo As String, ByVal estado As String) As Integer
        ...
        Try
           ...
           "(codigo, estado)" &
            ...
            End If
        Catch ex As Exception
            'MessageBox.Show("Error, Se presentó la siguiente exepción:" & ex.M)
        End Try
    ...
    '******************************
    '******************************
    ' RESERVAS o CERRAR PERIODO
    '******************************
    '******************************

    'Actualiza la info de Cerrar Periodo fechas
    Function actualizarCerrarPeriodoFechas(ByVal fechaDesde As Date, ByVal fechaHasta As Date) As Integer
        Dim res As Integer = 0
        Try
            'Declaramos el query que queremos ejecutar
            SQL = "UPDATE [CERRAR_PERIODO_FECHAS] SET fechaDesde = '" & fechaDesde & "', " & "fechaHasta = '" & fechaHasta & "' "
            'pregunto antes si estoy conectado a la base de datos'
            If conectadoBD = True Then
                Dim command As New OleDbCommand(SQL, objConexion)
                res = command.ExecuteNonQuery()
    ...
    Function obtenerDatosdeCerrarPeriodoFechas() As List(Of CerrarPeriodoFechasClase)
        Dim MyList As New List(Of CerrarPeriodoFechasClase)
        Try
            SQL = "SELECT CERRAR_PERIODO_FECHAS.* FROM [CERRAR_PERIODO_FECHAS]"
            'pregunto antes si estoy conectado a la base de datos'
            If conectadoBD = True Then
                Dim command As New OleDbCommand(SQL, objConexion)
                Dim reader = command.ExecuteReader()
    ...
    Function noRetirarAcumuladoCertificadoEnTransito(ByVal idAsoc As String, ByVal ubicacionEstado As String) As Integer
        Dim res As Integer = 0
        Try
            SQL = " UPDATE [CERTIFICADO_EN_TRANSITO] SET  estado= '" & ubicacionEstado & "'
                    WHERE (cedula = '" & idAsoc & "') "
            If conectadoBD = True Then
                Dim command As New OleDbCommand(SQL, objConexion)
                res = command.ExecuteNonQuery()
